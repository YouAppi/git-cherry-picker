...
class GitCommandExecutor {
    ...
    public GitCommandResult execute(GitCommand gitCommand) {
        ...
        if (verbose) {
            printOut.println '[DEBUG] executing gitCommand \'' + commandToExecute + '\''
        }
    ...
    public GitCommand gitLogCommand(String branchName) {
        gitCommand("git log -10000 $branchName --pretty=format:|~entry~||~author~|%an|~!author~||~commit_date~|%cd|~!commit_date~||~message_body~|%b|~!message_body~||~message_subject~|%s|~!message_subject~||~!entry~|")
    }
